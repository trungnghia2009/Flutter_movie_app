...
import 'package:flutterappmovie/bloc/shows/get_trailers_bloc.dart';
import 'package:flutterappmovie/bloc/shows/get_airing_today_shows_bloc.dart';
...
class _AiringTodayState extends State<AiringToday> {
  ...
  void initState() {
    super.initState();
    airingTodayShowsBloc..getAiringToday();
  }

  @override
  Widget build(BuildContext context) {
    print('AiringToday() build');
    return Column(
      ...
      children: <Widget>[
        ...
        StreamBuilder<ShowResponse>(
          stream: airingTodayShowsBloc.subject.stream,
  ...
  Widget _buildNowPlayingWidget(ShowResponse data) {
    ...
    if (shows.length == 0) {
      ...
      return Container(
        ...
        child: PageIndicatorContainer(
          ...
          length: data.shows.take(10).length,
        ),
      );
    }
  }

  Future<void> _buildTrailerDialog(ShowResponse data, int index) {
    final controller = PageController();

    trailersBloc.getTrailers(data.shows[index].id);
